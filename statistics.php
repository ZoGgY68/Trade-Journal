...
<body>
    <div class="container">
        ...
        <!-- Date Range Search -->
        <h2>Display Trades by Date Range</h2>
        <div class="filters">
            <form method="GET" action="statistics.php">
                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['stats_csrf_token']); ?>">
        ...
        <?php
        if (isset($_GET['start_date']) && isset($_GET['end_date']) && isset($_GET['csrf_token']) && $_GET['csrf_token'] === $_SESSION['stats_csrf_token']) {
            $start_date = filter_var($_GET['start_date'], FILTER_SANITIZE_STRING);
            $end_date = filter_var($_GET['end_date'], FILTER_SANITIZE_STRING);

            $stmt = $pdo->prepare('SELECT * FROM trades WHERE user_id = ? AND trade_date BETWEEN ? AND ? ORDER BY trade_date ASC');
            $stmt->execute([$_SESSION['user_id'], $start_date, $end_date]);
            $date_trades = $stmt->fetchAll();
            
            if ($date_trades) {
                $date_total_pl = array_sum(array_column($date_trades, 'profit_loss'));
                $date_wins = count(array_filter($date_trades, function($t) { return $t['profit_loss'] > 0; }));
                $date_losses = count(array_filter($date_trades, function($t) { return $t['profit_loss'] < 0; }));
                $date_win_rate = count($date_trades) > 0 ? ($date_wins / count($date_trades)) * 100 : 0;
                ?>
                <h2>Trades from <?php echo htmlspecialchars($start_date); ?> to <?php echo htmlspecialchars($end_date); ?></h2>
                
                <div class="stats-container">
                    <div class="stats-card">
                        <div class="stats-header">Period P&L</div>
                        <div class="stats-value <?php echo $date_total_pl > 0 ? 'positive' : 'negative'; ?>">
                            <?php echo number_format($date_total_pl, 2); ?>
                        </div>
                    </div>
                    <div class="stats-card">
                        <div class="stats-header">Win Rate</div>
                        <div class="stats-value">
                            <?php echo number_format($date_win_rate, 2); ?>%
                        </div>
                    </div>
                    <div class="stats-card">
                        <div class="stats-header">Total Trades</div>
                        <div class="stats-value">
                            <?php echo count($date_trades); ?>
                        </div>
                        <div><?php echo $date_wins; ?> wins / <?php echo $date_losses; ?> losses</div>
                    </div>
                </div>
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td><?php echo number_format($trade['price'], 2); ?></td>
                        <td><?php echo number_format($trade['exit_price'], 2); ?></td>
                        <td><?php echo number_format($trade['stop_loss'], 2); ?></td>
                        <td><?php echo number_format($trade['take_profit'], 2); ?></td>
                        <td class="<?php echo $trade['profit_loss'] > 0 ? 'positive' : ($trade['profit_loss'] < 0 ? 'negative' : ''); ?>">
                            <?php echo number_format($trade['profit_loss'], 2); ?>
                        </td>
                        <td><?php echo htmlspecialchars($trade['strategy']); ?></td>
